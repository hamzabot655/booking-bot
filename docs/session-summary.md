# Session Summary — June 30, 2026 (Part 4) — Full Vercel Cleanup, SPA Routing, GitHub Updates

## Summary
- **All Vercel projects wiped**: Deleted `goethe-frontend-v2` and `frontend` projects from Hamza's account. Created ONE fresh project `goethe-frontend-v2`.
- **SPA routing fixed**: Added `vercel.json` with rewrites rule (`{"source":"/(.*)","destination":"/index.html"}`) — subpaths like `/settings`, `/logs` now return 200 instead of 404.
- **Old domain deprecated**: `goethe-booking-dashboard.vercel.app` was an old project name — Vercel SSO intercepts it even after deletion. Using `goethe-frontend-v2.vercel.app` as primary URL.
- **GitHub secret updated**: `VERCEL_PROJECT_ID` set to `prj_jRIrDFcw3I2SDoAWEW78OGpIB0LY` via GitHub API using classic token.
- **All local changes pushed**: login fix, sw.js fix, AGENTS.md, session-summary, database.py signatures, db.py cleanup, pk_fallback.

## Files Changed
| File | Change |
|------|--------|
| `frontend/vercel.json` | NEW — SPA rewrites (`/*` → `/index.html`) |
| `frontend/.vercel/project.json` | Updated to new project ID `prj_jRIrDFcw3I2SDoAWEW78OGpIB0LY` |
| `AGENTS.md` | Updated URLs, project ID, Part 4 context |
...
## Remaining Items
- [ ] Fix Auto-Connect: remove `#backendUrl` input, auto-connect from saved URL
- [ ] Fix Railway Postgres: ensure `DATABASE_URL` auto-injected, remove duplicate DBs
- [ ] WebSocket auth security fix
- [ ] India adaptation (separate project)
- [ ] Run full QA + security audit
